...
class getanc:
    ...
    def __init__(self, file=None,
        ...
        #Asurin 010617: nasa changed this from http to https
        ...
        if self.file is None:
            ...
            if self.start is None:
                ...
                if not os.path.exists(self.file):
                    ...
                    if (re.search('(Aqua|Terra)', senchk)) and self.sensor is None:
                    #                    if self.mission == "A" or self.mission == "T":
                        self.start, self.stop, self.sensor = modis_timestamp(self.file)
                    elif re.search("viirs", senchk) and self.sensor is None:
    ...
    def findweb(self):
        ...
        if self.stop is None:
            dlstat = ProcUtils.httpdl(''.join([self.query_site,
                                               ...
                                               msnchar, opt_flag]),
                os.path.abspath(os.path.dirname(self.server_file)), outputfilename=self.server_file)
        else:
	    print ''.join([self.query_site,
                                               '/sdpscgi/public/display_ancillary_files.cgi?', 'type=',
                                               anctype,
        ...
        for line in ad:
            ...
            value = value.strip()
            if re.search('(met|ozone|file|att\d|eph\d)', key, re.IGNORECASE):
                key = key.lower()
        ...
        for FILE in FILES:
            ...
            if download == 1:
                if self.dl is False:
                    ...
                    status = ProcUtils.httpdl(''.join([self.data_site, '/cgi/getfile/', FILE]), self.dirs['path'],
                        uncompress=True)
                    gc.collect()
                    if status:
